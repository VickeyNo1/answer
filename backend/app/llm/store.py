...
def get_model(model_id: int) -> dict | None:
    with get_db_ctx() as db:
        cursor = db.execute("SELECT * FROM model_configs WHERE id = ?", (model_id,))
        row = cursor.fetchone()
...
def get_model_by_name(model_name: str) -> dict | None:
    with get_db_ctx() as db:
        cursor = db.execute(
            "SELECT * FROM model_configs WHERE model_name = ?", (model_name,)
        )
        row = cursor.fetchone()
        return _row_to_model(row) if row else None
...
def create_model(provider: str, model_name: str, display_name: str,
    ...
    with get_db_ctx() as db:
        ...
        db.commit()
        cursor = db.execute("SELECT * FROM model_configs WHERE id = ?", (cursor.lastrowid,))
...
def update_model(model_id: int, fields: dict) -> dict | None:
    ...
    with get_db_ctx() as db:
        cursor = db.execute("SELECT id FROM model_configs WHERE id = ?", (model_id,))
        if cursor.fetchone() is None:
            return None
        if updates:
            params.append(model_id)
            db.execute(f"UPDATE model_configs SET {', '.join(updates)} WHERE id = ?", params)
            db.commit()
        cursor = db.execute("SELECT * FROM model_configs WHERE id = ?", (model_id,))
        return _row_to_model(cursor.fetchone())
...
def activate_model(model_id: int) -> dict | None:
    with get_db_ctx() as db:
        ...
        db.execute(
            "UPDATE model_configs SET is_active = 1, enabled = 1 WHERE id = ?",
            (model_id,),
        )
        db.commit()
        cursor = db.execute("SELECT * FROM model_configs WHERE id = ?", (model_id,))
...
def get_usage_stats(days: int = 7) -> dict:
    ...
    with get_db_ctx() as db:
        ...
        row = cursor.fetchone()
        total_tokens, total_cost = row["tokens"], round(row["cost"], 6)

        # 今日
        cursor = db.execute(
            """SELECT COALESCE(SUM(total_tokens), 0) AS tokens, COALESCE(SUM(cost), 0) AS cost
               FROM usage_logs WHERE DATE(created_at) = DATE('now', 'localtime')"""
        )
        row = cursor.fetchone()
        today_tokens, today_cost = row["tokens"], round(row["cost"], 6)
        ...
        by_model = [
            {"model_name": r["model_name"], "tokens": r["tokens"], "cost": round(r["cost"], 6)}
        ...
        day_map = {r["d"]: (r["tokens"], round(r["cost"], 6)) for r in cursor.fetchall()}
